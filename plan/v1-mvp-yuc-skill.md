...
- 解析与标准化模块负责统一输出模型，不绑定具体外部数据源。
- yuc 能力是底层确定性数据源代码，负责请求 yuc 页面、提取源记录并调用标准化模块。
...
1. `modules/anime-parser/PLAN.md`：只描述外部数据源无关的解析与标准化输出模型；
2. `modules/yuc-scraper/PLAN.md`：只描述 yuc.wiki 静态抓取能力；
3. `plan/v1-agent-skill.md`：只描述本地 Agent Skill / Tool 创建。
...
详见 `modules/anime-parser/PLAN.md`。
...
详见 `modules/yuc-scraper/PLAN.md`。
...
### 4.3 V1-B：本地 Agent Skill / Tool 创建
