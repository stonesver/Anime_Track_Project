...
class TestYucServiceGetWeeklySchedule:
    ...
    def test_weekly_schedule_uses_anime_parser_normalized_shape(self):
        """Test yuc records are normalized through anime-parser."""
        import os
        fixture_path = os.path.join(os.path.dirname(__file__), "fixtures", "season_202604.html")
        with open(fixture_path, "r", encoding="utf-8") as f:
            html = f.read()
        ...
        result = service.get_weekly_schedule(season="2026-04", weekday=3)

        assert result.ok is True
        assert result.data["source"] == "yuc"
        assert result.data["season"] == "2026-04"
        assert len(result.data["days"]) == 1

        item = result.data["days"][0]["items"][0]
        assert item["source"] == "yuc"
        assert item["season"] == "2026-04"
        assert item["title_cn"] == "动漫名称"
        assert "Anime Name" in item["aliases"]
...
class TestYucServiceSearch:
    ...
    def test_search_uses_anime_parser_aliases(self):
        """Test search can match aliases generated by yuc to anime-parser adaptation."""
        import os
        fixture_path = os.path.join(os.path.dirname(__file__), "fixtures", "season_202604.html")
        with open(fixture_path, "r", encoding="utf-8") as f:
            html = f.read()
        ...
        result = service.search_anime(query="Anime Name", season="2026-04", limit=5)

        assert result.ok is True
        assert len(result.data) == 1
        assert result.data[0]["title_cn"] == "动漫名称"
